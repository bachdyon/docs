...
```mermaid
flowchart TD
  jobMgr[video-job-manager]
  vds[video-design-spec-builder]
  plan[video-creative-planner]
  voice[ausynclab-voice]
  dedupeGate{can_lam_sach_lap_loi?}
  dedupe[audio-deduplicate]
  timing[word-timestamps-extractor]
  branchNhaHinh{co_anh_AI_hoac_thieu_footage?}
  fal[fal-image-generator]
  index[asset-semantic-extractor]
  mapper[semantic-asset-mapper]
  coverage[shot-coverage-planner]
  rplan[video-render-plan-builder]
...
- `audio-deduplicate` chỉ chạy khi voice bị lặp lời và cần làm sạch.
- `fal-image-generator` chỉ chạy khi video cần ảnh AI hoặc không có đủ footage.
