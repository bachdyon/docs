...
**Prerequisites**: `faster-whisper` trong Python environment đang chạy. `OPENAI_API_KEY` trong `.env` chỉ cần nếu chạy `$word-timestamps-extractor` với `--backend openai`.
...
1. `$word-timestamps-extractor` trích xuất transcript có timestamp cấp câu/từ từ file audio. Script mặc định chạy local `faster-whisper` → TOML word-level timestamps.
