...
`video-automator-skills` là kho kỹ năng (skills) để xây pipeline sản xuất video ngắn bằng agent: lập kế hoạch, xử lý audio, ánh xạ asset, dựng timeline và render. Mỗi skill đóng gói một bước trong pipeline, có thể tái sử dụng giữa các job.
